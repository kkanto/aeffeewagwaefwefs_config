...
def job_config = [
        job: [
                name: "aeffeewagwaefwefs_Pipeline_develop",
                agent: "maven"
        ],
        git: [
                branch: "develop"
...
def customLib = library identifier: 'aeffeewagwaefwefs_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/kkanto/aeffeewagwaefwefs_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = customLib.de.allianz.aeffeewagwaefwefs.new()
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                   jslGit.checkout( config, "kkanto", "aeffeewagwaefwefs", job_config.git.branch)
               }
            }    
        }
        stage('Build') {
...
def job = pipelineJob("aeffeewagwaefwefs_Pipeline_develop");

job.with {

    authenticationToken('DhtvmzBh0B1ycrs8C6yK5tbsjGBQbmHN')
